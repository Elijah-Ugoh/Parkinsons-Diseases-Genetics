...
for _, row in score_df.iterrows():
    ...
    if key in bim_lookup:
        ...
        if row["effect_allele"] == bim_ref and row["other_allele"] == bim_alt:  # Exact match found
            updated_snps.append([bim_snp_id, row["effect_allele"], row["effect_weight"], row["rsID"]])
...
updated_score_df = pd.DataFrame(updated_snps, columns=["rsID", "effect_allele", "effect_weight", "rsID"])
...
updated_score_df.to_csv("updated_1805_scores.txt", sep="\t", index=False, header=False)

# Save unmatched SNPs **with headers** for manual review
unmatched_snps_df.to_csv("unmatched_1805_snps.txt", sep="\t", index=False, header=True)
...
print(f"Updated PRS score file saved as: updated_1805_scores.txt\n"
      f"Unmatched SNPs saved to: unmatched_1805_snps.txt")
